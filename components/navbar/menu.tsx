...
const Menu = () => {
  ...
  const textDisplay = "Advertise  (Soon)";
  return (
    <div className="hidden lg:flex items-center relative">
      <div className="space-x-6 text-gray-600 text-sm flex items-center cursor-pointer">
        ...
        >
          Community (Soon) {showCommunityMenu && <CommunityMenu />}
        </div>

        <div className="hover:text-[#ff6154] py-4"></div>
        {textDisplay}
        <div
          ...
          className="hover:text-[#ff6154] py-4"
        >
          About (Soon) {showAboutMenu && <AboutMenu />}
        </div>
      </div>
    </div>
  );
