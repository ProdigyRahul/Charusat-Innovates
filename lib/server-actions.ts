  ...
  try {
    ...
    await db.notification.create({
      data: {
        ...
        body: `Your Project ${product.name} has been activated`,
        type: "ACTIVATED",
        status: "UNREAD",
...
export const rejectProduct = async (productId: string, reason: string) => {
  try {
    ...
    await db.notification.create({
      data: {
        userId: product.userId,
        body: `Your project "${product.name}" has been rejected. Reason: ${reason}`,
        type: "REJECTED",
        status: "UNREAD",
        profilePicture: `${product.logo}`,
        productId: productId,
    ...
    if (productDetails && productDetails.userId !== userId) {
      ...
      await db.notification.create({
        data: {
          ...
          body: `Commented on your Project "${productDetails.name}"`,
          profilePicture: profilePicture,
          productId: productId,
          type: "COMMENT",
          status: "UNREAD",
...
export const getUpvotedProducts = async () => {
  try {
    ...
    console.error("Error getting upvoted Projects:", error);
    return [];
  }
};
